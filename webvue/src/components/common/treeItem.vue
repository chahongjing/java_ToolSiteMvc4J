  ...
  <li :class='{"tree-item":true,"folder":isFolder}'>
    <div class='tree-item-content'>
      ...
      <span :class='{"tree-item-text":true,"selected":model.selected}' @click.stop='option.clickItem(model)'>
       ...
       <span class="tree-item-text-content" v-html='model.name'></span>
     </span>
      <span class="tree-btn-group fr">
        <i class="fa fa-plus text-primary" @click="option.addNode(model)"></i>
        <i class="fa fa-edit text-primary" @click="option.editNode(model)"></i>
        <i class="fa fa-trash text-primary" @click="option.deleteNode(model)"></i>
      </span>
   </div>
...
<style scoped>
  ...
  /* 71 33 */
  span.tree-item-text{font-size:0;padding:0 3px 0 1px;width:calc(100% - 71px);height:21px;line-height: 21px;vertical-align: middle;}
  span.tree-item-text.selected .tree-item-text-content{background-color: #83b5f3;border-radius:1px;}
  .tree-item-icon{
    width:15px;
    text-align: center;
    height:21px;line-height: 21px;vertical-align: middle;
  }
  .tree-item-text-content{white-space:nowrap;
    overflow:hidden;
  text-overflow:ellipsis;
  white-space:nowrap; width:100%;}
  .tree-item-text-icon{width:15px;text-align:center;}
  ...
  ul ul > li:after{top:11px;left:-5px;width:20px;border-top-width:1px;}
  ...
  ul > li:last-child:before{content:' ';position:absolute;top:0;left:-5px;height:12px;border-left-width:1px;}
  ...
  .radio_checkbox input + i:after{left:2px;}
  .radio_checkbox, .togglecheckbox{margin-right:18px;}
  .tree-btn-group i{display:none;}
  .tree-item-content:hover .tree-btn-group i{display:inline-block;}
</style>
